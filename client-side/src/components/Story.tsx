...
const Story: React.FC<PropWords> = ({ currentWords }): ReactElement => {
	const {
		petName,
		wizardName,
		creature1,
	...
	return (
		<div className={`story-container`}>
			<p>
				...
				detriment, as they would be so happy partaking in this hobby that they would hardly
				think of anything else.
			</p>
			<p>
				<u>{petName || "name"}</u> decided one day to venture out and bring their friend to
			...
			<p>
				For this journey, <u>{petName || "name"}</u> packed up several{" "}
				<u>{noun3 || "pluralNoun"}</u>, and at the last moment (<u>{timeOfDay || "time"}</u>
				) they were finally able to drag <u>{wizardName || "name"}</u> outside and to their
				adventure.
			</p>
			<p>
				The trail from their <u>{place2 || "place2"}</u> to the <u>{place1 || "place"}</u>{" "}
				was <u>{adj2 || "adj"}</u>, where beings of magic and trickery would appear out of
				seemingly nowhere, and do anything to get an unfortunate traveler's attention. One
				such being, a <u>{creature2 || "creature"}</u>, <u>{pastV1 || "pastTenseVerb"}</u>{" "}
				out in front of our wizard and <u>{creature1 || "creature1"}</u> duo, and said to
				them “<u>{phrase1 || "phrase"}</u>”
			</p>
			<p>
			...
			<p>
				...
				<u>{specialty || "specialty"}</u>. They were enthralled in the ever wonderful
				subject they loved so dearly, and despite <u>{petName || "name"}</u> constantly
				tugging on their <u>{outfit || "outfit"}</u>, once they started sharing they would
				not stop.{" "}
			</p>
			<p>
				As the story continued, the wizard began to notice the{" "}
				<u>{creature2 || "creature"}</u> was peering elsewhere, where they spotted another
			...
			<p>
				...
				<u>{emotion || "emotion"}</u>. Never before had they experienced their excitement
				for <u>{specialty || "specialty"}</u> in such a way. They looked at the{" "}
				<u>{noun1 || "noun"}</u> and <u>{noun2 || "noun"}</u>, the supposed wonders of this
